...
Claude Code 的 hooks 机制可以做到：AI 每次改代码，lint 自动运行，低级问题自动修复。
...
Claude Code 支持后台运行：用 `run_in_background` 参数，或者 Bash 的后台模式。AI 在后台跑测试、重构模块，你去做 code review、开会、写别的代码。完成后通知你。
