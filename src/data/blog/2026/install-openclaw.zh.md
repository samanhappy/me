...
> Oracle 的"永久免费"有个反直觉的细节：很多人担心实例会被回收，但实际上 Oracle 主要回收的是长期空闲的实例。OpenClaw 本身就是持续负载（心跳、定时任务），反而不容易被判定为空闲——折腾一次注册，长期用的稳定性其实比想象中高。
...
购买服务器时选择 OpenClaw 应用镜像，启动后通过 SSH 本地转发访问管理后台（安全起见不推荐直接开放 18789 端口）：

```bash
ssh -N -L 18789:127.0.0.1:18789 root@<公网IP>
```
...
| 端口 | 用途 |
|------|------|
| 18789 | OpenClaw 管理后台（**安全起见不建议直接开放**，见下文） |
| 443 | HTTPS（Webhook 回调） |
| 22 | SSH 管理 |

> **安全提示**：18789 是 OpenClaw 的管理后台，暴露在公网等于把 Agent 的控制权拱手送人。建议用 SSH 本地转发代替开放端口：
>
> ```bash
> ssh -N -L 18789:127.0.0.1:18789 root@你的服务器IP
> ```
>
> 执行后在本地浏览器打开 `http://localhost:18789` 就能访问管理后台，全程走 SSH 加密隧道，无需开放 18789 端口。
...
你可能注意到表格里 Gemini 2.5 Flash（$0.15/$0.60）的输入价格和 DeepSeek 几乎一样。那为什么不选 Gemini？两个原因：一是 DeepSeek 的输出价格只有 Gemini 的一半（$0.28 vs $0.60），OpenClaw Agent 场景下输出量不小；二是缓存价格差了 14 倍（$0.0028 vs $0.04）——OpenClaw 的系统提示词、技能定义每次推理都要重复发送，缓存命中率很高，这个差距在实际使用中会被显著放大。

顺带一提 GLM 4.5 Air（免费）。能用吗？能。推理能力和上下文窗口有限，如果你只是尝鲜，可以从它开始零成本入门。但如果你想让 Agent 真正帮你解决复杂问题——查资料、写代码、做决策——免费的体验会让你很快想升级。DeepSeek V4 Flash 一个月才 $2.6，不值得为了省这个钱忍受一个不够聪明的模型。

### 别忘了关掉 Token 黑洞：心跳机制

到这里，你的 OpenClaw 已经能跑起来了。服务器成本固定，模型成本可控。但还有一个隐藏的吞金兽——**心跳（Heartbeat）**。

OpenClaw 默认每 30 分钟自动执行一次心跳：Agent 醒来，读取 `HEARTBEAT.md` 检查有没有需要关注的事（邮件、日历、系统状态），一切正常就回个 `HEARTBEAT_OK` 继续睡。听起来很贴心，但有一个致命的默认行为——**心跳运行在主 Session 中，携带完整的对话历史**。

如果你的对话已经跑了几百轮，每次心跳都要把几万 token 的历史上下文发给模型。算一下：

```
心跳频率：每 30 分钟 → 每天 48 次
每次消耗：对话历史 + 系统提示词 ≈ 20,000–50,000 token
每天消耗：48 × 35,000 ≈ 1,680,000 token
按 DeepSeek V4 Flash 算：1.68M × $0.14 ≈ $0.24/天 → $7/月
```

光心跳一天就烧掉 168 万 token——是你前面预估的"日均 50 万"的三倍。这意味着你实际花的 API 费用可能远高于 $2.6/月。
...
`isolatedSession: true` 是核心——每次心跳创建全新会话，不带历史上下文。加上 `lightContext: true` 只加载 `HEARTBEAT.md`，每次心跳的 token 消耗从 35,000 降到约 3,500——成本直接砍到 **1/10**。

优化后的心跳成本：

```
每天消耗：48 × 3,500 ≈ 168,000 token
每天成本：0.168M × $0.14 ≈ $0.023/天
每月成本：$0.023 × 30 ≈ $0.70/月
```

从 $7/月降到不到 $1/月。如果你暂时不需要后台巡逻，直接设 `every: "0m"` 关掉心跳，省掉这最后一块钱。

关于心跳的更多细节——和 Cron 的本质区别、`HEARTBEAT.md` 怎么写、tasks 子任务怎么配——见[《OpenClaw 心跳机制：让你的 AI 学会自主巡逻》](/blog/openclaw-heartbeat)。
...
|------|---------|
| 腾讯云轻量服务器（境外） | ~8 元 |
| DeepSeek V4 Flash API（日常对话） | ~18 元（$2.6） |
| 心跳监控（优化后） | ~5 元（$0.70） |
| **合计** | **~31 元/月** |
