...
系统通过 **Bindings**（消息绑定）来决定"来自哪个 Channel 的哪个用户的消息，应该由哪个 Agent 处理"。这是一种确定性路由规则，可以基于 channel 类型、账号、用户 ID、群组、角色等条件进行匹配。
...
3. Gateway 根据 Bindings 找到应该处理这条消息的 Agent
...
这是一个容易被忽视但很重要的设计。当 Agent 正在处理消息时，如果又有新消息进来，系统不是等它跑完再处理，而是通过 **Steering Queue**（消息转向队列）决定如何处理这条新消息。

OpenClaw 提供了六种队列模式：

- **steer**（默认）：将新消息注入当前正在运行的 Agent，让模型在下一个推理边界看到它
- **queue**：旧版逐个注入模式
- **steer-backlog**：注入当前运行，同时保留一份给后续 followup
- **followup**：不打扰当前运行，排队等它跑完再处理
- **collect**：等当前运行结束后，将多等待消息合并成一个 followup 回合
- **interrupt**：中断当前运行，立刻处理最新消息

这个机制的另一面是串行保证：同一个 Session 内的消息永远不会并行处理，避免了并发读写文件导致的竞态问题。稍微慢一点，但换来了确定性和可调试性。
...
Bindings 是将消息路由到不同 Agent 的确定映射规则。一个 Binding 本质上是一组匹配条件 `(channel, accountId, peer, guild/team) → agentId`，告诉 Gateway "这条消息该交给哪个 Agent 处理"。

路由遵循 **most-specific wins** 原则：peer 级别匹配优先于 channel 级别匹配，具体规则优先于通配规则。比如你可以让某个特定 WhatsApp 联系人的消息路由到专属 Agent，而其他所有 WhatsApp 消息走默认 Agent。

Bindings 让 OpenClaw 的多 Agent 架构成为可能——你可以同时运行"工作助手"和"生活管家"等多个 Agent，每个有自己独立的 workspace、SOUL.md 和 session 存储。
